...
public class LineChartActivity2 extends DemoBase implements //OnSeekBarChangeListener,
    ...
    public static final String NH_3 = "NH3";
...
//        List<DataVo>dataVoList=   finalDb.findAllByWhere(DataVo.class,
//                String.format(" sensorIndex=%s and " +
//                        "time=%s and gasIndex=%s"
//                        ,sensorIndex,dateLong,gasIndex));
//        if(dataVoList==null){
...
//                dataVo.setGasIndex(gasIndex);
//                dataVo.setTime(dateLong);
//                dataVo.setIndex(i);
//                int val = (int) (Math.random() * 10) + uplimit-9;
//                dataVo.setVal();
//            }
//        }
        ...
        for (int i=0,lenth=datasetTitles.size();i<lenth;i++){
            ...
            int uplimit=upLimits.get(i);
            int gasIndex=0;
            if(CH_4.equals(datasetTitles.get(i))){
                gasIndex=DataVo.CH4;
            }else  if(NH_3.equals(datasetTitles.get(i))){
                gasIndex=DataVo.NH3;
            ...
            for (int j = 0; j < count; j++) {
                DataVo dataVo= finalDb.findById(DataVo.generateId(dateLong,sensorIndex,gasIndex,j),DataVo.class);
//                float mult = (range + 1);
                int val = (int) (Math.random() * 10) + uplimit-9;// + (float)
                if (dataVo==null){
                    dataVo=new DataVo(dateLong, j,sensorIndex, gasIndex, val);
                    finalDb.save(dataVo);
                }
                val=dataVo.getVal();
                // ((mult *
...
//    public static final String SENSOR_INDEX="sensorIndex";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String str1CH4=SharedPreferenceUtil.get(LineChartActivity2.this,SettingActivity.UPLIMITCH4,String.valueOf(SettingActivity.UPLIMITCH4_DEFAULT));
        String str1NH3=SharedPreferenceUtil.get(LineChartActivity2.this,SettingActivity.UPLIMITNH3,String.valueOf(SettingActivity.UPLIMITNH3_DEFAULT));
        String strH2S=SharedPreferenceUtil.get(LineChartActivity2.this,SettingActivity.UPLIMITH2S,String.valueOf(SettingActivity.UPLIMITH2S_DEFAULT));
//        String str2=SharedPreferenceUtil.get(LineChartActivity2.this,SettingActivity.DAYS_KEEPED,String.valueOf(SettingActivity.DAYS_KEEPED_DEFAULT));
        ...
        ArrayList<Integer>integerArrayList=getIntent().getIntegerArrayListExtra(SearchActivity.SENSOR_INDEX_LIST_INTEGER);
        sensorIndex=integerArrayList.get(0);
...
//            mChart.notifyDataSetChanged();
//        } else {
        if(System.currentTimeMillis()-dateLong>24*3600*1000){//过去的时间
            generateVals(24, 80);

        }else {
            Calendar calendar=Calendar.getInstance();
            int count= calendar.get(Calendar.HOUR_OF_DAY);
            generateVals(count+1, 80);
        }
