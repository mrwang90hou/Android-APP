...
public class FinalDb {
    ...
    private static FinalDb currentFinalDb;
	...
	private synchronized static FinalDb getInstance(DaoConfig daoConfig) {
		...
		//如果之前的数据库不是要操作的数据库，需要重新检查表的存在与否
		if (currentFinalDb!=dao) {
	 ...
	 * @methods execSQL
	 * @description 执行sql
	 * @date 2014-12-5 下午12:04:06
	 * @param sql
	 *            参数说明
	 */
	public void execSQL(Class<?> clazz, String sql) {
		if (!TextUtils.isEmpty(sql)) {
			checkTableExist(clazz);
			debugSql(sql);
			db.execSQL(sql);
		} else {
			Log.e(TAG, "execSQL error:sql is null");
		}
	}
	 ...
	 *            参数说明
	 */
	public int execUpdate(Object obj, String table, ContentValues values, String whereClause, String[] whereArgs) {
